...
export class rscManager {
  ...
  public async getRsc(src: string) {
    if (this.mRscObject[src]) return this.mRscObject[src]
    const { ok, data } = await jsonApi.getRsc(src)
    if (ok) {
      this.mRscObject[src] = await PIXI.Assets.load(data)
      return this.mRscObject[src]
    }
  }
  // /**
  //  * @param src 리소스
  ...
  //   try {
  //     this.mRscObject[`${scene}/${src}`] = await PIXI.Assets.load(
  //       `${link}/rsc/${sceneName}/img/${src}`
  //     )
  //   } catch (e) {
  //     console.error(e)
  //   }
